...
public sealed class User : BaseEntity, IAggregateRoot
{
    ...
    public static User Create(string email, string passwordHash, string displayName, Role role = Role.Viewer)
    {
        ...
        return new User(email.Trim().ToLowerInvariant(), passwordHash, displayName, role);
    }
